...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates the surface-info scaffold defaults"
    (rf/dispatch-sync [:initialize-db])
    (is (= app/default-db @rf-db/app-db))
    (is (= "Kouza Core K0uz401" @(rf/subscribe [:title])))
    (is (= "etzhayyim-project-kouza" @(rf/subscribe [:project])))
    (is (= "kouza-core-k0uz401" @(rf/subscribe [:name])))
...
(deftest routes-sub-reflects-db
  (testing ":routes subscription reads whatever is in the db, not a fixed value"
    (reset! rf-db/app-db {:routes ["/health" "/xrpc/foo"]})
    (is (= ["/health" "/xrpc/foo"] @(rf/subscribe [:routes])))))

(deftest vars-sub-reflects-db
  (testing ":vars subscription reads whatever is in the db, not a fixed value"
    (reset! rf-db/app-db {:vars ["APP_NANOID" "APP_DISPLAY_NAME"]})
    (is (= ["APP_NANOID" "APP_DISPLAY_NAME"] @(rf/subscribe [:vars])))))
...
(deftest initialize-db-overwrites-prior-state
  (testing ":initialize-db resets to defaults even if the db already had other data"
    (reset! rf-db/app-db {:title "stale" :routes ["stale"] :unrelated 42})
    (rf/dispatch-sync [:initialize-db])
    (is (= app/default-db @rf-db/app-db))))
